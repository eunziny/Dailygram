					...
					<c:if test="${not empty list }">
						<c:forEach var="p" items="${list }">
							<li class="list-group-item"><c:choose>
								...
								href='${pageContext.request.contextPath }/board/friList.do?writer=${p.id }'
								style="color: black"><span>${p.id }</span></a> <a type="button"
								href='${pageContext.request.contextPath }/friend/cancelFollow.do?writer=${p.id }&type=1'
								class="btn btn-xs">팔로잉 취소</a></li>
						</c:forEach>
					</c:if>
				</ul>
			</div>
